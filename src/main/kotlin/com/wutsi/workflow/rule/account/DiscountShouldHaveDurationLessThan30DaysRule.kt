package com.wutsi.workflow.rule.account

import com.wutsi.error.ErrorURN
import com.wutsi.marketplace.access.dto.Discount
import com.wutsi.platform.core.error.Error
import com.wutsi.platform.core.error.exception.ConflictException
import com.wutsi.workflow.Rule

class DiscountShouldHaveDurationLessThan30DaysRule(private val discount: Discount) : Rule {
    override fun check() {
        if (discount.ends.minusDays(30).isAfter(discount.starts)) {
            throw ConflictException(
                error = Error(
                    code = ErrorURN.DISCOUNT_DURATION_TOO_LONG.urn,
                    data = mapOf(
                        "discount-id" to discount.id,
                        "discount-start-date" to discount.starts,
                        "discount-end-date" to discount.ends,
                    ),
                ),
            )
        }
    }
}
